...
void MpcOsqp::CalculateGradient() {
  ...
  std::cout << "Gradient_mat"<<std::endl;
...
void MpcOsqp::CalculateEqualityConstraint(std::vector<c_float> *A_data,
  ...
  Eigen::MatrixXd state_identity_mat = Eigen::MatrixXd::Identity(
      state_dim_ * (horizon_ + 1), state_dim_ * (horizon_ + 1));
  std::cout << "state_identity_mat: " <<std::endl;
  std::cout <<  state_identity_mat <<std::endl;
  ...
  std::cout << "matrix_constraint with A"<<std::endl;
  std::cout << matrix_constraint<<std::endl;
  ...
  std::cout << "matrix_constraint with B"<<std::endl;
  std::cout << matrix_constraint<<std::endl;
  ...
  std::cout << "matrix_constraint with I"<<std::endl;
  std::cout << matrix_constraint<<std::endl;
...
void MpcOsqp::CalculateConstraintVectors() {
  ...
  std::cout << " matrix_u_lower_"<<std::endl;
  for (size_t i = 0; i < horizon_ + 1; i++) {
  ...
  std::cout << " matrix_x_lower_"<<std::endl;
  ...
  std::cout << " matrix_initial_x_"<<std::endl;
...
OSQPData *MpcOsqp::Data() {
  ...
  if (data == nullptr) {
    ...
    std::cout << "CalculateKernel done"<<std::endl;
    data->P =
        csc_matrix(kernel_dim, kernel_dim, P_data.size(), CopyData(P_data),
                   CopyData(P_indices), CopyData(P_indptr));
    std::cout << "Get P matrix"<<std::endl;
    data->q = gradient_.data();
    std::cout << "before CalculateEqualityConstraint"<<std::endl;
    std::vector<c_float> A_data;
    std::vector<c_int> A_indices;
    std::vector<c_int> A_indptr;
    CalculateEqualityConstraint(&A_data, &A_indices, &A_indptr);
    std::cout << "CalculateEqualityConstraint done"<<std::endl;
    data->A =
